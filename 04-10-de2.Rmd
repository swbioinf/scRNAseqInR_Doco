...
The example below quickly looks at 3 different methods on the Kang 2018 PBMC dataset ....
...
DimPlot(kang) + DimPlot(kang.celltype)
```
...
pseudobulk_matrix <- AggregateExpression( kang.celltype,  slot = 'counts', assays='RNA' )[['RNA']]
...
ind  <- as.character(gsub(".*_","",colnames(pseudobulk_matrix))) # ind should be treated as text, not a number!
